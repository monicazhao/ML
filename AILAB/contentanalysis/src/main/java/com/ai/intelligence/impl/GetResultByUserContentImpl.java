...
import com.ai.intelligence.util.UrlEffectiveJudgement;
import com.ai.intelligence.util.UrlJudgement;
...
@Component("getResultByUserContentImpl")
public class GetResultByUserContentImpl implements IGetResultByUserContentPkgSV {
	...
	public JSONObject getUserContentResult(JSONObject userContent) {
		...
		Logger logger = Logger.getLogger(GetResultByUserContentImpl.class);
		...
		if(!urlJudger.isURL(contentStr)){	
			...
			//调用分析接口
			IContentAnalysisPkgSV contentAnalysis = new ContentAnalysisImpl();
