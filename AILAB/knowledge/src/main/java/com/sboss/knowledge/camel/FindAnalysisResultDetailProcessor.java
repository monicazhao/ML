...
public class FindAnalysisResultDetailProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		System.out.println("FindAnalysisResultDetailProcessor--->tempObj" + tempObj.toString());
		...
		System.out.println("FindAnalysisResultDetailProcessor--->keyWordsList" + keyWordsList);
		String keyWordsInter = keyWordsList.substring(1,keyWordsList.length()-1);
		System.out.println("FindAnalysisResultDetailProcessor--->keyWordsInter" + keyWordsInter);
		String[] keyWords = keyWordsInter.split(",");
		for(int i = 0; i < keyWords.length; i++){
			String keyWord = keyWords[i].split(":")[0];
			JSONObject singleKeyWord = new JSONObject();
			System.out.println("FindAnalysisResultDetailProcessor--->keyWord" + keyWord);
			singleKeyWord.put("analysis_keyword", keyWord.substring(1, keyWord.length()-1));
			analysisKeywordList.add(singleKeyWord);
			System.out.println("FindAnalysisResultDetailProcessor--->singleKeyWord" + singleKeyWord);
		}
